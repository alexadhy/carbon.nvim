local hsluv = require("carbon.hsluv")
...
-- local ns = vim.api.nvim_create_namespace("carbon")
...
function util.debug(colors)
  colors = colors or require("carbon.colors")
  -- Dump unused colors
...
function util.onColorScheme()
  if vim.g.colors_name ~= "carbon" then
    vim.cmd([[autocmd! carbon]])
    vim.cmd([[augroup! carbon]])
  end
end
...
function util.autocmds(config)
  vim.cmd([[augroup carbon]])
  vim.cmd([[  autocmd!]])
  vim.cmd([[  autocmd ColorScheme * lua require("carbon.util").onColorScheme()]])
  if config.dev then
    vim.cmd([[  autocmd BufWritePost */lua/carbon/** nested colorscheme carbon]])
  end
...
function util.terminal(colors)
  ...
  vim.g.terminal_color_14 = colors.cyan1
